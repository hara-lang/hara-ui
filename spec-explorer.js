...
export function createSpecExplorer(root, { manifestUrl = "spec-manifest.json", fileUrl = (path) => path, repositoryUrl } = {}) {
  const state = { files: [], directories: [], active: null, mode: "preview" };
  ...
  function renderTree(files, directories) {
    ...
    const visit = (node, parent, depth) => { [...node.directories.entries()].sort(([a], [b]) => a.localeCompare(b)).forEach(([name, child]) => { const details = document.createElement("details"); const empty = child.directories.size === 0 && child.files.length === 0; details.className = "hara-tree-directory"; details.open = !empty && depth < 1; details.setAttribute("aria-disabled", String(empty)); const summary = document.createElement("summary"); summary.textContent = name; if (empty) summary.addEventListener("click", (event) => event.preventDefault()); details.append(summary); visit(child, details, depth + 1); parent.append(details); }); node.files.sort((a, b) => a.path.localeCompare(b.path)).forEach((file) => { const button = document.createElement("button"); button.type = "button"; button.className = "hara-tree-file"; button.setAttribute("aria-current", String(state.active?.path === file.path)); button.innerHTML = `<i class="hara-file-dot ${file.kind}"></i><span>${fileName(file.path)}</span>`; button.addEventListener("click", () => activate(file)); parent.append(button); }); };
