...
export function createSpecExplorer(root, { manifestUrl = "spec-manifest.json", fileUrl = (path) => path, repositoryUrl } = {}) {
  ...
  function renderTree(files, directories) {
    ...
    const visit = (node, parent, depth, parentPath) => { [...node.directories.entries()].sort(([a], [b]) => a.localeCompare(b)).forEach(([name, child]) => { const path = parentPath ? `${parentPath}/${name}` : name; const details = document.createElement("details"); const empty = child.directories.size === 0 && child.files.length === 0; details.className = "hara-tree-directory"; details.open = !empty && depth < 1 && !state.collapsedDirectories.has(path); details.setAttribute("aria-disabled", String(empty)); const summary = document.createElement("summary"); summary.textContent = name; if (empty) summary.addEventListener("click", (event) => event.preventDefault()); else details.addEventListener("toggle", () => { if (details.open) state.collapsedDirectories.delete(path); else state.collapsedDirectories.add(path); }); details.append(summary); visit(child, details, depth + 1, path); parent.append(details); }); node.files.sort((a, b) => a.path.localeCompare(b.path)).forEach((file) => { const button = document.createElement("button"); button.type = "button"; button.className = "hara-tree-file"; button.setAttribute("aria-current", String(state.active?.path === file.path)); button.innerHTML = `<i class="hara-file-dot ${file.kind}"></i><span>${fileName(file.path)}</span>`; button.addEventListener("click", () => { const branch = file.path.split("/").slice(0, -1).join("/"); if (branch) state.collapsedDirectories.add(branch); activate(file); }); parent.append(button); }); };
