...
function treeFrom(files) {
  ...
  for (const file of files) { let node = root; for (const part of file.path.split("/").slice(0, -1)) { if (!node.directories.has(part)) node.directories.set(part, { directories: new Map(), files: [] }); node = node.directories.get(part); } node.files.push(file); }
  return root;
}
...
export function createSpecExplorer(root, { manifestUrl = "spec-manifest.json", fileUrl = (path) => path, repositoryUrl } = {}) {
  ...
  function renderTree(files) {
    ...
    const visit = (node, parent, depth) => { [...node.directories.entries()].sort(([a], [b]) => a.localeCompare(b)).forEach(([name, child]) => { const details = document.createElement("details"); details.open = depth < 1; const summary = document.createElement("summary"); summary.textContent = name; details.append(summary); visit(child, details, depth + 1); parent.append(details); }); node.files.sort((a, b) => a.path.localeCompare(b.path)).forEach((file) => { const button = document.createElement("button"); button.type = "button"; button.className = "hara-tree-file"; button.setAttribute("aria-current", String(state.active?.path === file.path)); button.innerHTML = `<i class="hara-file-dot ${file.kind}"></i><span>${fileName(file.path)}</span>`; button.addEventListener("click", () => activate(file)); parent.append(button); }); };
    visit(treeFrom(files), fragment, 0); elements.tree.replaceChildren(fragment);
  }
  function updateTree() { const query = elements.search.value.trim().toLowerCase(); const files = query ? state.files.filter((file) => file.path.toLowerCase().includes(query)) : state.files; elements.count.textContent = `${files.length} FILES`; if (!files.length) { elements.tree.innerHTML = '<p class="hara-explorer-empty">No matching files.</p>'; return; } renderTree(files); }
  async function activate(file, requestedMode) {
    state.active = file; state.mode = requestedMode || (file.kind === "markdown" ? "preview" : "source"); history.replaceState(null, "", `#/${encodeURIComponent(file.path)}`); elements.crumb.textContent = file.path; elements.title.textContent = fileName(file.path); elements.badge.hidden = false; elements.badge.textContent = file.kind; elements.raw.href = fileUrl(file.path); elements.repository.href = repositoryUrl ? `${repositoryUrl.replace(/\/$/, "")}/${file.path}` : fileUrl(file.path); elements.source.setAttribute("aria-pressed", String(state.mode === "source")); elements.preview.hidden = file.kind !== "markdown"; elements.preview.setAttribute("aria-pressed", String(state.mode === "preview")); elements.document.innerHTML = '<div class="hara-explorer-welcome"><p>Loading file…</p></div>'; updateTree();
  ...
  elements.search.addEventListener("input", updateTree); elements.source.addEventListener("click", () => state.active && activate(state.active, "source")); elements.preview.addEventListener("click", () => state.active && activate(state.active, "preview")); window.addEventListener("hashchange", () => { const file = state.files.find((candidate) => candidate.path === hashPath()); if (file && file.path !== state.active?.path) activate(file); });
  fetch(manifestUrl).then((response) => { if (!response.ok) throw new Error(`Manifest request failed with ${response.status}`); return response.json(); }).then((manifest) => { state.files = manifest.files || []; updateTree(); const file = state.files.find((candidate) => candidate.path === hashPath()) || state.files.find((candidate) => candidate.path === "README.md") || state.files[0]; if (file) return activate(file); }).catch((error) => { elements.document.innerHTML = `<div class="hara-explorer-welcome hara-explorer-error"><h2>Explorer unavailable</h2><p>${escapeHtml(String(error.message || error))}</p></div>`; });
