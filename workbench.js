...
export function createWorkbench(root, { model = coreFlowModel() } = {}) {
  const state = { model: clone(model), selected: null, armed: null, draft: null, next: model.nodes.length + 1 };
  ...
  el.canvas.addEventListener("pointermove", (event) => { if (state.draft) { const box = el.canvas.getBoundingClientRect(); state.draft.x = event.clientX - box.left; state.draft.y = event.clientY - box.top; render(); } });
  el.canvas.addEventListener("pointerup", (event) => { if (!state.draft) return; const target = event.target.closest(".hara-port.in"); const from = state.draft.from; state.draft = null; if (target && compatible(state.model, from, target.dataset.portId)) { const connection = { id: `c/${state.model.connections.length + 1}`, from, to: target.dataset.portId }; state.model.connections.push(connection); setStatus("MODEL // STALE", "stale"); emit("structural-edit", { kind: "connect", connection, model: clone(state.model) }); } else if (target) { setStatus("INCOMPATIBLE PORTS", "error"); emit("rejected-edit", { kind: "connect", from, to: target.dataset.portId }); } render(); });
